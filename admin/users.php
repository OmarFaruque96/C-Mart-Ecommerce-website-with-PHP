...
generateStringBtn.addEventListener('click', () => randomNumber.innerHTML = generateRandomString(8));
      ...
      if($data == 'edit'){
        $editid = $_GET['editid'];
        $users = mysqli_query($db,"SELECT * FROM mart_users WHERE ID='$editid'");
        while($row = mysqli_fetch_assoc($users)){
            $id             = $row['ID'];
            $firstname      = $row['firstname'];
            $lastname       = $row['lastname'];
        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST'){
           ...
           if(!empty($pass) && !empty($file_name)){
            ...
            if($file){
                ...
                $userUpdateSql = "UPDATE mart_users SET username = '$name', email = '$email', pass = '$hasspass', address = '$address', photo = '$updatedname', phone = '$phone', status = '$status' WHERE ID = '$id'";
           ...
           if(!empty($pass) && empty($file_name)){
                ...
                $userUpdateSql = "UPDATE mart_users SET username = '$name', email = '$email', pass = '$hasspass', address = '$address', phone = '$phone', status = '$status' WHERE ID = '$id'";
                $update_res = mysqli_query($db, $userUpdateSql);
                if($update_res){
                    header('location: users.php');
                }else{
           ...
           if(empty($pass) && !empty($file_name)){
            ...
            if($file){
              ...
              $userUpdateSql = "UPDATE mart_users SET username = '$name', email = '$email',  address = '$address', photo = '$updatedname', phone = '$phone', status = '$status' WHERE ID = '$id'";
              $update_res = mysqli_query($db, $userUpdateSql);
              if($update_res){
                  header('location: users.php');
              }else{
           ...
           if(empty($pass) && empty($file_name)){
              $userUpdateSql = "UPDATE mart_users SET username = '$name', email = '$email',  address = '$address', phone = '$phone', status = '$status' WHERE ID = '$id'";
              $update_res = mysqli_query($db, $userUpdateSql);
              if($update_res){
                  header('location: users.php');
              }else{
